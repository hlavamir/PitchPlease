...
#define NUM_LEDS_RECEIVE 19 // since the patterns are symmetrical, only receiving half of the data
...
void standaloneBehaviour(){
  ...
  for(int stripCrr = 0; stripCrr < NUM_STRIPS; stripCrr++){
    ...
    for(int j = 0; j < NUM_LEDS; j++){
      leds[stripCrr][j] = CHSV(idleHue[i], idleSaturation, idleBrightness);
    }
  }
