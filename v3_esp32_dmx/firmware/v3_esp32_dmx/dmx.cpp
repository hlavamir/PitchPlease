...
#define DMX_SERIAL_INPUT_PIN        16 // pin for dmx rx (RO)
#define DMX_SERIAL_INPUTENABLE_PIN  32 // pin for dmx rx/tx change

#define DMX_SERIAL_OUTPUT_PIN       17 // pin for dmx tx (DI)
#define DMX_SERIAL_OUTPUTENABLE_PIN 12 // pin for dmx rx/tx change

#define DMX_SERIAL_IO_PIN           4  // pin for dmx rx/tx change
...
void DMX::DisableDuringSetup(){
    pinMode(DMX_SERIAL_INPUTENABLE_PIN, OUTPUT);
    digitalWrite(DMX_SERIAL_INPUTENABLE_PIN, HIGH);
}
...
void DMX::Initialize(DMXDirection direction)
{
    ...
    //pinMode(DMX_SERIAL_IO_PIN, OUTPUT); // LOW - receive, HIGH - transmit
    pinMode(DMX_SERIAL_INPUTENABLE_PIN, OUTPUT);
    //pinMode(DMX_SERIAL_OUTPUTENABLE_PIN, OUTPUT);

    digitalWrite(DMX_SERIAL_INPUTENABLE_PIN, LOW);
    //digitalWrite(DMX_SERIAL_OUTPUTENABLE_PIN, HIGH);

    // depending on parameter set gpio for direction change and start rx or tx thread
    if(direction == output)
    {
        //digitalWrite(DMX_SERIAL_IO_PIN, HIGH);
    ...
    {    
        //digitalWrite(DMX_SERIAL_IO_PIN, LOW);
        dmx_state = DMX_IDLE;
                ...
                case UART_DATA:
                    ...
                    if(dmx_state == DMX_DATA)
                    {
                        ...
                        for(int i = 0; i < event.size; i++)
                        {
                            ...
                            {
                                if(dmx_data[current_rx_addr] != dtmp[i])
                                {
                                    OnDMXInput(current_rx_addr, dtmp[i]);
                                } 
