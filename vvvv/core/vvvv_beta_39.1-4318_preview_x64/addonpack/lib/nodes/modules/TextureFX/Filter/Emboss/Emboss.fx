...
float MapSharp;
float3 lDir <string uiname="Light Direction";> = {2, -1, 4};
...
float4 p0(float2 vp:vpos):color{float2 x=(vp+.5)/R;
    ...
    float3 n=float3(length(tex2Dlod(s1,float4(x-float2(1,0)*off,0,lod)).rgb)-length(tex2Dlod(s1,float4(x+float2(1,0)*off,0,lod)).rgb),length(tex2Dlod(s1,float4(x-float2(0,1)*off,0,lod)).rgb)-length(tex2Dlod(s1,float4(x+float2(0,1)*off,0,lod)).rgb),0);
    n.z=-max(abs(n.x),abs(n.y));
    n=normalize(n)*pow(length(n),pow(2,MapSharp));
    float lgt=dot(n.xyz,normalize(lDir))*BumpAmount;
    if(!Colored)c.rgb=.5;
    //c.rgb=lerp(c.rgb*(1+lgt),c.rgb+lgt,lgt>0);
    c.rgb+=lgt;
    return c;
}
